...
export type CityKey = "la" | "tokyo" | "hk";
...
export interface ReleaseMerchant {
  ...
  /** IANA timezone from the city config — the per-merchant tz source for booking datetimes and call windows (multi-city breaks the global LAUNCH_TZ; see AGENT-UX item 1 / @analyst's REQ-FUL-5 flag). */
  timezone: string;
...
export interface ReleaseManifest {
  ...
  dropped: { non_restaurant: number; unnamed: number; no_geometry: number; out_of_bbox: number; no_address: number; invalid_phone: number };
